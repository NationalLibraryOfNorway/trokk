...
import {
    ...
    response400Mock,
    textInputDtoResponseMockNewspaper,
} from './mock-data.mock';
...
describe('RegistrationSchema.svelte', () => {
    ...
    beforeEach(() => {
        ...
        mockIPC((cmd, args) => {
            switch (cmd) {
                ...
                case 'tauri': {
                    if (args['__tauriModule'] === 'Http') {
                        // fetch requests has cmd=tauri and args.__tauriModule=Http
                        return Promise.resolve(
                            textInputDtoResponseMockNewspaper,
                        );
                    }
    ...
    test('registration should get hostname from backend', async () => {
        ...
        expect(hostNameSpy).toHaveBeenCalledWith(
            expect.objectContaining({ cmd: 'get_hostname' }),
        );
    });
    ...
    test('registration should move files to done directory', async () => {
        ...
        expect(hostNameSpy).toHaveBeenCalledWith(
            expect.objectContaining({ cmd: 'copy_dir' }),
        );
    });
    ...
    test('registration should get size of files for new directory', async () => {
        ...
        expect(hostNameSpy).toHaveBeenCalledWith(
            expect.objectContaining({
                cmd: 'get_total_size_of_files_in_folder',
            }),
        );
    });
    ...
    test('registration should get access token for PAPI', async () => {
        ...
        expect(tokenSpy).toHaveBeenCalledWith(
            expect.objectContaining({ cmd: 'get_papi_access_token' }),
        );
    });
                    ...
                    options: expect.objectContaining({
                        body: expect.objectContaining({
                            payload: expect.objectContaining({
                                createdBy:
                                    authenticationResponseMock.userInfo.name,
                                fileSize: 123n,
                                font: 'ANTIQUA',
    ...
    test('registration should delete old files after successful registration', async () => {
        ...
        await new Promise((resolve) => setTimeout(resolve, 0));
        expect(hostNameSpy).toHaveBeenCalledWith(
            expect.objectContaining({ cmd: 'delete_dir' }),
        );
    });
    ...
    test('registration should show success message', async () => {
        ...
        await new Promise((resolve) => setTimeout(resolve, 0));
        const res = container.getByText('sendt til produksjonsløypen', {
            exact: false,
        });
    ...
    test('registration should show error message if file copy failed', async () => {
        ...
        await new Promise((resolve) => setTimeout(resolve, 0));
        const res = container.getByText('Fikk ikke flyttet filene', {
            exact: false,
        });
        expect(res).toBeTruthy();
    });
    ...
    test('registration should show error message if file size retrieval copy failed', async () => {
        mockIPC((cmd) => {
            if (cmd === 'get_total_size_of_files_in_folder')
                return Promise.reject('file size failed');
            else return '';
        });
        ...
        await new Promise((resolve) => setTimeout(resolve, 0));
        const res = container.getByText('Fikk ikke hentet filstørrelse', {
            exact: false,
        });
        expect(res).toBeTruthy();
    });
    ...
    test('registration should show error message if get token for papi failed', async () => {
        mockIPC((cmd) => {
            if (cmd === 'get_papi_access_token')
                return Promise.reject('token failed');
            else return '';
        });
        ...
        await new Promise((resolve) => setTimeout(resolve, 0));
        const res = container.getByText(
            'Kunne ikke hente tilgangsnøkkel for å lagre objektet i databasen',
            { exact: false },
        );
        expect(res).toBeTruthy();
    });
    ...
    test('registration should show error message if papi post failed', async () => {
        ...
        await new Promise((resolve) => setTimeout(resolve, 0));
        const res = container.getByText('Kunne ikke TRØKKE dette videre', {
            exact: false,
        });
        expect(res).toBeTruthy();
    });
