...
import { authenticationResponseMock, envVariablesMock, fileEntryListMock } from "./mock-data.mock";
...
describe('App.svelte', () => {
    ...
    test('logout should clear interval for refreshing token, show login, login should be invoked on click', async () => {
        ...
        await waitFor(() => expect(tauriIpcSpy).toHaveBeenCalledWith(expect.objectContaining({cmd: 'log_in'})))
    })
