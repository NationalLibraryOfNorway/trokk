...
describe('Auth.svelte', () => {
    ...
    describe('component', () => {
        ...
        beforeEach(async () => {
            setAuthResponseSpy = vi.spyOn(settings, 'authResponse', 'set').mockReturnValue()
            vi.useFakeTimers()
        })
        ...
        test('component mounts', () => {
            container = render(Auth, {props: {authResponse: null, loggedOut: false}})
        ...
        test('login works from launch', async () => {
            ...
            const getEmptyAuthSpy = vi.spyOn(settings, 'authResponse', 'get').mockReturnValue(Promise.resolve(null))
            ...
            await waitFor(async () => {
                ...
                expect(tauriIpcSpy).toHaveBeenCalledWith(expect.objectContaining({ cmd: "log_in" }));
            })

            // Since we cannot trigger the event from the component in tests, we need to call the event handler directly
            container.component.handleTokenExchangedEvent(loginWindowMock).call(null, {
            ...
            await waitFor(() => expect(setAuthResponseSpy).toHaveBeenCalledWith(authenticationResponseMock))
        ...
        test('should use saved login on launch', async () => {
            const getAuthSpy = vi.spyOn(settings, 'authResponse', 'get').mockReturnValue(Promise.resolve(authenticationResponseMock))
            const tauriIpcSpy = vi.spyOn(window, '__TAURI_IPC__')
            ...
            await waitFor(async () => {
                await awaitNthTicks(50);
                expect(setAuthResponseSpy).toHaveBeenCalledWith(authenticationResponseMock);
            })

            // Expect the refresh interval to be set
            expect(vi.getTimerCount()).toBe(1)
            // Expect a refresh token call
            expect(tauriIpcSpy).toHaveBeenCalledWith(expect.objectContaining({cmd: 'refresh_token'}))
