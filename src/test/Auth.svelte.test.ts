...
describe('Auth.svelte', () => {
        ...
        mockIPC((cmd, args) => {
            switch (cmd) {
                ...
                case 'get_required_env_variables':
                    return Promise.resolve(envVariablesMock);
                case 'refresh_token':
    ...
    describe('component', () => {
        ...
        test('login works from launch', async () => {
            ...
            let loginWindowMock = getAll().filter(
                (w) => w.label === 'Login',
            )[0];
    ...
    describe('module', () => {
        ...
        test('refresh should save new token', async () => {
            ...
            await refreshAccessToken();
            expect(authResponseSpy).toHaveBeenCalledWith(
                authenticationResponseMock,
            );
        });
        ...
        test('canRefresh should return true if refresh token is not expired', async () => {
            const authMock = structuredClone(authenticationResponseMock);
            authMock.expireInfo.refreshExpiresAt =
