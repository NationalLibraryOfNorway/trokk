        ...
        stopWatching = await watch(
            ...
            async () => {
                if (currentPath && !(await exists(currentPath)))
                    currentPath = undefined;
                fileTree = await getFileEntries();
                const currentEntry: FileTreeType | undefined =
                    findCurrentDir(fileTree);
                if (currentEntry) {
    ...
    async function getFileEntries(): Promise<FileTreeType[]> {
        return await readDir(scannerPath, { recursive: true })
            .then(async (newFiles) => {
                ...
                let firstDir = newFileEntries.find(
                    (file: FileTreeType): boolean => {
                        return !!file.children;
                    },
                );
                if (currentPath)
                    currentPath = (await exists(currentPath))
                        ? currentPath
                        : firstDir?.path ?? undefined;
            ...
            .catch((err) => {
                console.error(
                    `An error occurred when reading directory \'${scannerPath}\': ${err}`,
                );
                readDirFailed = err;
                return [];
            });
    ...
    <div class="files-container">
        <div id="left-pane" data-testid="left-pane" class="pane sticky-top">
            ...
            <FileTree
                ...
                on:directoryChange={(event) =>
                    changeViewDirectory(event.detail)}
            />
        </div>
            ...
            <div class="images">
                    ...
                    {#each viewFiles as viewFile}
                        {#if viewFile.fileTree.children}
                            <button
                                class="directory"
                                on:click={() =>
                                    changeViewDirectory(viewFile.fileTree)}
                            >
                                <Folder size="96" />
                                <i
                                    >{viewFile.imageSource
                        ...
                        {:else if supportedFileTypes.includes(getFileExtension(viewFile.imageSource))}
                            <div>
                                <img
                                    src={viewFile.imageSource}
                                    alt={viewFile.fileTree.name}
                                />
                                <i
                                    >{formatFileNames(
                                        viewFile.imageSource
                ...
                {:else}
                    <p class="dir-help-text">
                        ...
                        Er det ingen mapper, sjekk at det fins filer i den valgte
                        scanner kilden.
                    </p>
                {/if}
            </div>
        ...
        <div id="right-pane" class="pane sticky-top">
            {#if currentPath}
                <RegistrationSchema bind:currentPath />
            {/if}
        </div>
    </div>
{:else}
