<script lang="ts">
    ...
    onMount(async () => {
        papiPath = (
            (await invoke('get_required_env_variables')) as RequiredEnvVariables
        ).papiPath;
    });
    ...
    async function postRegistration(scanner: string): Promise<void> {
        ...
        const newPath = await copyToDoneDir(id).catch((error) => {
            handleError(
                'Fikk ikke flyttet filene, sjekk at mappeinnstillinger er korrekt.',
            );
            return Promise.reject(error);
        });

        const fileSize = await getTotalFileSize(newPath!).catch((error) => {
            deleteDir(newPath!);
            handleError('Fikk ikke hentet filstørrelse.');
            return Promise.reject(error);
        });

        const accessToken = await invoke('get_papi_access_token').catch(
            (error) => {
                deleteDir(newPath!);
                handleError(
    ...
    function getTotalFileSize(path: string): Promise<BigInt> {
        return invoke('get_total_size_of_files_in_folder', { path: path }).then(
            (size: BigInt) => size,
        );
    }
...
<form class="regContainer" on:submit|preventDefault={onSubmit}>
    ...
    <div class="regField">
        <label for="workingTitle">
            Arbeidstittel (Blir ikke brukt i produksjon)
        </label>
        <input
            type="text"
            name="workingTitle"
