<script context="module" lang="ts">
    ...
    export async function refreshAccessToken(): Promise<AuthenticationResponse | void> {
        ...
        if ((await canRefresh()) && authResponse) {
            return invoke<AuthenticationResponse>('refresh_token', {
                refreshToken: authResponse.tokenResponse.refreshToken,
            }).then((res) => {
    ...
    export async function setRefreshAccessTokenInterval(
        ...
        if (!authResponse)
            throw new Error(
                'Cannot set refresh token interval: User not logged in',
            );
    ...
    export async function login(): Promise<string | void> {
        ...
        return invoke('log_in').then(async (port) => {
            ...
            let loginWebView = new WebviewWindow('Login', {
                url:
                    envVars.oidcBaseUrl +
                    '/auth?scope=openid&response_type=code&client_id=' +
                    envVars.oidcClientId +
            ...
            await appWindow.once(
                'token_exchanged',
                handleTokenExchangedEvent(loginWebView),
            );
        });
    }

    export function handleTokenExchangedEvent(
        loginWebView: WebviewWindow,
    ): EventCallback<unknown> {
        return async (event) => {
            authResponse = event.payload as AuthenticationResponse;
            settings.authResponse = authResponse;
            setRefreshAccessTokenInterval(authResponse).then((id) => {
