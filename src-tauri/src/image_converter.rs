...
pub fn set_image_size_fractions(
	...
	if !(MIN_SIZE_FRACTION..=MAX_SIZE_FRACTION).contains(&thumbnail_fraction) {
		return Err(format!(
			"Invalid thumbnail fraction: {thumbnail_fraction}. Must be between 1 and 16."
		));
	}
	if !(MIN_SIZE_FRACTION..=MAX_SIZE_FRACTION).contains(&preview_fraction) {
		return Err(format!(
			"Invalid preview fraction: {preview_fraction}. Must be between 1 and 16."
		));
	}
