...
fn test_copy_dir_should_copy_old_dir_with_contents_and_give_it_new_name() {
    ...
    DirBuilder::new().create(&old_dir).expect("Failed to create old directory");
    ...
    DirBuilder::new().create(&subdir).expect("Failed to create subdir");

    let done_dir = base_path.path().join("done");
    DirBuilder::new().create(&done_dir).expect("Failed to create new directory");
...
fn test_delete_dir_should_delete_directory() {
    let dir = TempDir::with_prefix("trokk-test-delete-tmp-").expect("Failed to create temp directory");
